...
def save_pwd():
    ...
    credential = vault_class.Credentials()
    ...
    if website_data == '' or pwd_data == '':
        ...
        if is_ok:
            ...
            dictObj = credential.read_file()

            # Add new credentials to vault
            dictObj.update(credential.website_data)

            # Write the data to Vault file
            credential.wrire_file(dictObj)
            clear_fields()
...
def search_website():
    try:
        ...
        if website_data == '':
            messagebox.showinfo(title="Website", message="The website field is empty for search!")

    credential = vault_class.Credentials()
    # Read the existing Vault file
    dictObj = credential.read_file()
    if website_data in dictObj:
        creds = dictObj[website_data]
        messagebox.showinfo(title="Credentials", message=f"Email: {creds['login']},\n"
                                                    f"Password: {creds['password']}")
    else:
        messagebox.showinfo(title="Website", message=f"The vault doesn't have credentials for website {website_data}.")
